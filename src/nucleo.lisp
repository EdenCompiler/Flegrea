...
(defclass object-3d ()
  ((position :initarg :position :initform (make-vector3) :accessor position)
   ...
   (quaternion :initarg :quaternion :initform (make-quaternion) :accessor quaternion)
   ...
   (camadas :initarg :layers :initform 1 :accessor flegrea:layers)
   (ordem-de-renderizacao :initarg :render-order :initform 0 :accessor flegrea:render-order)
   (corte-por-frustum :initarg :frustum-culled :initform t :accessor flegrea:frustum-culled)
   (projeta-sombra :initarg :cast-shadow :initform nil :accessor flegrea:cast-shadow)
   (recebe-sombra :initarg :receive-shadow :initform nil :accessor flegrea:receive-shadow)
   (dados-do-usuario :initarg :user-data :initform (make-hash-table :test #'equal)
                     :accessor flegrea:user-data)
   (caixa-limite :initarg :bounding-box :initform nil :accessor flegrea:bounding-box)
   (esfera-limite :initarg :bounding-sphere :initform nil :accessor flegrea:bounding-sphere)))
...
(defun set-rotation (objeto x y z &optional (order (order (rotation objeto))))
  ...
  (set-euler (rotation objeto) x y z order)
  (set-from-euler (quaternion objeto) (rotation objeto))
  objeto)
(defun flegrea:set-object-quaternion (objeto x y z w)
  "Altera o quaternion local e sincroniza a rotação Euler."
  (flegrea:set-quaternion (quaternion objeto) x y z w)
  (flegrea:set-from-quaternion (rotation objeto) (quaternion objeto))
  objeto)
...
(defun update-matrix (objeto)
  ...
  (set-from-euler (quaternion objeto) (rotation objeto))
  (compose-matrix4 (matrix objeto) (position objeto) (quaternion objeto) (scale objeto))
  objeto)
